...
export default function Button({
  ...
  const widthClass = width && width.startsWith('w-') ? width : 'w-60';

  return (
    <button
      className={`text-center rounded-lg px-7 py-1 text-base cursor-pointer ${bgClass} ${textClass} ${widthClass} ${className}`}
      style={style}
      {...props}
    >
      {children}
